...
import { LogType, TopicNode, VideoClassStatus, QuestionStatus, CCAStatus, ReviewStatus, StudyLog, INITIAL_NODES, INITIAL_LOGS } from "./mockData";
...
function NodeRow({ node, tree, collapsed, logs, onToggle, onUpdate, onOpenDetail, depth = 0 }: RowProps) {
        ...
        <tr className="cursor-pointer select-none" onClick={() => onToggle(node.id)}>
          <td colSpan={9} className="py-1.5 px-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-base text-indigo-700 underline underline-offset-2 tracking-wide">{node.name}</span>
              <span className={`ml-1 w-2 h-2 rounded-full ${ccaDot(node.status)}`} />
            </div>
          </td>
        </tr>
        {!isCollapsed && children.map((c) => <NodeRow key={c.id} node={c} tree={tree} collapsed={collapsed} logs={logs} onToggle={onToggle} onUpdate={onUpdate} onOpenDetail={onOpenDetail} depth={depth + 1} />)}
      </>
    );
  }
      ...
      <>
        <tr className="bg-slate-50/80 border-y border-slate-200">
          <td className="py-1 px-2 w-6">
            <button onClick={() => onToggle(node.id)} className="w-5 h-5 flex items-center justify-center rounded hover:bg-slate-200 transition-colors">
              <span className={`text-xs text-slate-500 transition-transform ${isCollapsed ? "" : "rotate-90"}`}>▶</span>
            </button>
          </td>
          <td colSpan={8} className="py-1 text-xs font-semibold text-slate-600 uppercase tracking-wider" style={{ paddingLeft: `${depth * 16 + 8}px` }}>{node.name}</td>
        </tr>
        {!isCollapsed && children.map((c) => <NodeRow key={c.id} node={c} tree={tree} collapsed={collapsed} logs={logs} onToggle={onToggle} onUpdate={onUpdate} onOpenDetail={onOpenDetail} depth={depth + 1} />)}
      </>
    );
    ...
    <>
      <tr className="group border-b border-slate-100 hover:bg-indigo-50/40 transition-colors">
        <td className="py-1.5 px-2 w-6">
          {hasChildren && <button onClick={() => onToggle(node.id)} className="w-5 h-5 flex items-center justify-center rounded hover:bg-slate-200"><span className={`text-xs text-slate-500 transition-transform ${isCollapsed ? "" : "rotate-90"}`}>▶</span></button>}
        </td>
        ...
        <table className="w-full border-collapse min-w-[860px]">
          ...
          <tbody>
            {filteredRoots.map((root) => (
              <NodeRow key={root.id} node={root} tree={tree} collapsed={collapsed} logs={logs}
